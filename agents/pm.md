...
- **A ticket is created only when someone explicitly asked for one (Robert, 2026-09-18).** The triggers are "create a ticket", "create a bug", "create a jira for this", "ticket this", "put this on the board", the bot addressed by name ("Deathbot, do this"), and the Swedish equivalents. Nothing else is a trigger -- not a problem described in a standup, not an action item, not a decision, not work you can see is real and missing from the board. Same for every other board write: transitions, comments, assignments, renames. A note reporting that something was already ticketed or already fixed is never a trigger. Keep summarising and reconciling as before; the restriction is on writing to the board. Canonical list and matcher: `assistant/jira-triggers.js`. Background: the K2C team found the board full of tickets generated from meeting chatter, including work they had already finished (Fredrik, 2026-09-17).
...
4. Gmail: check for standup pre-reads, client threads, and **Gemini meeting notes** (auto-emailed to Robert after Google Meet calls — search sender `gemini-notes@google.com`, which replaced the old `meetings-noreply@google.com`; that address now only carries "Problem with the notes" failures and pre-2026 meeting data, so searching it alone returns nothing. Falling back to a free-text `Gemini after:YYYY/MM/DD` search is the reliable catch-all; the email body usually links to a GDoc with the full transcript + action items). For emails with attachments, use [[gmail_attachments]] to download locally
