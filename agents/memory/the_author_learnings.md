...
## 2026-09-18 - Kelly Zmak prep mail v2 (Kelly absent weeks 7 to 9): the counterpart's absence is a deadline, not an opener, and priority is carried by order plus one "Your call" [exel_riyadh / bg-001, mail register, my edits unverified]
- **Do not open by naming the counterpart's absence; put the fact inside the deadline clause.** "Saw you are away for weeks 7 to 9. Let us take that call before you travel" became "Taking you up on the call, this week before you travel." Same ask, the absence carried by "before you travel", nothing left that can read as reproach. Same move as the retraction-in-the-opener rule: a fact that only exists to justify the ask goes inside the ask. Also "Let us" is not his; "Taking you up on" is (pass 1, mirrors Kelly's own offer). Category: register (opener).
- **Two jabs cut that a drafter writes as precision:** "depending on which of your mails I read" (after 22 or 23 teams) and "while you are still reachable". Both are true, both act on how Kelly reads Robert, neither changes what he decides. "22 or 23 teams" alone surfaces the discrepancy neutrally. Category: register (anti-selling, the pointed-precision variant).
- **Priority without a label = order plus the one decision sentence.** The two must-happen-before-he-travels items (1:1 format, app access) moved to the top, and the 1:1 block ends "Your call, but I want it settled before you go." Cut "so this is on the critical path" (positional label, 2026-08-31 rule); "I am holding the deck build until I can see the previous mentors' decks" already carries the blocking. Category: structure.
- **"You offered X and I will take you up on it" with no concrete ask underneath is hollow; replace with his open-door form.** Became "I will ping you if the login gives me trouble" (his "Ping me / Shout if" construction, mirroring Kelly's own "let me know if you have trouble finding the invite", thread `1a0abca02b16ee17`, 18 Sep). Note for the parent: that same mail says Robert is "pending" in the app with the invite in his inbox, so the next action on access is his, not Kelly's; the mail states the blocking fact without asking Kelly to do anything. Category: register.
- **A section that bundles a routing question with two resource asks: keep one section, make each ask its own sentence, cut the enumeration lead-in.** "Two things I need lined up there:" would have been the "Two smaller ones:" cut; "For the Monday I need ... For the Wednesday, one team ..." carries both without a signpost. The "is the new EIR the person to take the asks to" sub-question merged into "Who do I coordinate with day to day while you are away?", since it is the same question pre-answered. Category: structure.
